...
def main():
    ...
    if cmd.model not in ["NorESM2-LM", "NorESM2-MM"]:
        raise ValueError("This script only valid for NorESM2-LM"
            + f" or NorESM2-MM, not {cmd.model}")
    ...
    # For this model, the ocean grid has one extra row of data
    # on axis = 1 (y direction) compared to the sea ice data.
    # The model documentation explains the issue and that for 
    # sea ice data the final row of any corresponding ocean grid
    # variable (here areacello) should be dropped.
    # 
    # Reference:
    # 
    # https://noresm-docs.readthedocs.io/en/noresm2/faq/
    ...
    for m in range(n_ens):
        ...
        sie_n[:,m] = np.nansum(
            sie_mask*areacello[np.newaxis,:,:]
                    *lat_mask_n[np.newaxis,:,:],
            axis=(1,2))
        
        sie_s[:,m] = np.nansum(
            sie_mask*areacello[np.newaxis,:,:]
                    *lat_mask_s[np.newaxis,:,:],
            axis=(1,2))
        
        sia_n[:,m] = np.nansum(
            sia_mask*areacello[np.newaxis,:,:]
                    *lat_mask_n[np.newaxis,:,:],
            axis=(1,2))
