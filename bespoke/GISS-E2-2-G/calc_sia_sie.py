...
def main():
    ...
    # Load areacella (siconc is provided on the atmosphere
    # grid, i.e. "siconca"):
    _, lat, areacella = lpd.areacella(cmd.model)
    ...
    for m in range(n_ens):
        ...
        sie_n[:,m] = np.nansum(
            sie_mask*areacella[np.newaxis,:,:]
            *lat_mask_n[np.newaxis,:,np.newaxis],
            axis=(1,2))
        
        sie_s[:,m] = np.nansum(
            sie_mask*areacella[np.newaxis,:,:]
            *lat_mask_s[np.newaxis,:,np.newaxis],
            axis=(1,2))
        
        sia_n[:,m] = np.nansum(
            sia_mask*areacella[np.newaxis,:,:]
            *lat_mask_n[np.newaxis,:,np.newaxis],
            axis=(1,2))
